...
Component({
  ...
  pageLifetimes: {
    // 组件所在页面的生命周期函数
    show: function () {
      console.log("show------=---", timer, ctx, this.data.countStatus);
      ctx = wx.createCanvasContext('bubble', this);
      queue = {};
      app.globalData.bool = false;
    },
    hide: function () {
      console.log("hide------=---", timer, ctx, this.data.countStatus)
  ...
  methods: {
    ...
    bubbleAnimate() {
      ...
      Object.keys(queue).forEach(key => {
        ...
        if (y > height && !this.data.countStatus) {
          delete queue[anmationData.id];
        }
      });
      ctx.draw();
      if (!this.data.countStatus) {
        queue = {};
      }
      if (Object.keys(queue).length > 0) {
        timer = setTimeout(() => {
